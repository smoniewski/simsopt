...
__all__ = ['GaussianSampler', 'GaussianSamplerInformed', 'PerturbationSample', 'CurvePerturbed', 
           'RotationSample', 'AssembledCurve', 'CurvePerturbedInformed', 'GaussianSampler1D']
...
class GaussianSampler1D(GSONable):
    ...
    def draw_sample(self, randomgen=None):
        ...
        if randomgen is None:
            randomgen = np.random
        z = randomgen.standard_normal(size=(n*(n_derivs+1), 1))
        curve_and_derivs = self.L@z
        return [curve_and_derivs[(i*n):((i+1)*n), :] for i in range(n_derivs+1)]
...
class CurvePerturbed(sopp.Curve, Curve):
    ...
    def __init__(self, curve, sample):
        ...
        self.sample = sample
        # print(np.mean(np.linalg.norm(self.sample[0][:])))
...
class CurvePerturbedInformed(sopp.Curve, Curve):

    """A perturbed curve."""

    def __init__(self, curve, sample_stoch, sample_extra, weight, option, offset):
        r"""
        Perturb a underlying :mod:`simsopt.geo.curve.Curve` object by drawing a perturbation from a
        :obj:`GaussianSampler`.
        ...
        sopp.Curve.__init__(self, curve.quadpoints)
        Curve.__init__(self, depends_on=[curve])
        self.sample_stoch = sample_stoch
        self.sample_extra = sample_extra
        if option == None:
            print("NO OPTION SELECTED")
            return(0)
        if option == 'T':
            self.info = 1/max(curve.curve.torsion())*np.array([curve.curve.torsion()])
        elif option == 'C':
            self.info = curve.curve.kappa()/max(curve.curve.kappa())
        perturbations = np.zeros((len(self.info),3))
        for i in range(len(self.info)):
            A = (self.offset+self.weight*(self.info[i]+self.sample_extra[0][i]))
            # print(np.linalg.norm(A))
            perturbations[i][:] = A*self.sample_stoch[0][i][:]
        self.new_sample = perturbations
    ...
    def gamma_impl(self, gamma, quadpoints):
        assert quadpoints.shape[0] == self.curve.quadpoints.shape[0]
        assert np.linalg.norm(quadpoints - self.curve.quadpoints) < 1e-15
        gamma[:] = self.curve.gamma() + self.new_sample[:]

    def gammadash_impl(self, gammadash):
        gammadash[:] = self.curve.gammadash() + self.sample_stoch[1]

    def gammadashdash_impl(self, gammadashdash):
        gammadashdash[:] = self.curve.gammadashdash() + self.sample_stoch[2]

    def gammadashdashdash_impl(self, gammadashdashdash):
        gammadashdashdash[:] = self.curve.gammadashdashdash() + self.sample_stoch[3]
